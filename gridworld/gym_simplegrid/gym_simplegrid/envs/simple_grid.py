...
class SimpleGridEnv(Env):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(
        ...
        desc: list[str] = None,
        map_name: str = None,
        reward_map: dict[bytes, float] = None,
        p_noise: float = None,
        wind_p: float = 0.0,
        **kwargs,
    ):
        ...
        ----------
        desc: list[str]
            Custom map for the environment. If it is an int, this is assumed to be the desired size of a randomly generated map
        map_name: str
        ...
        p_noise: float
            ...
            - P(move right)=.1
        wind_p: float
            Probability of wind forcing you into a certain direction
        """
        ...
        self.reward_range = (
            min(self.reward_map.values()),
            max(self.reward_map.values()),
        )
    ...
    def __initialise_grid_from_desc(self, desc: list[str]) -> SimpleGrid:
        """
            ...
            for col in range(ncol):
                ...
                for a in range(nA):
                    ...
                    if letter in b"GWB":
                        li.append(
                            (1.0, s, 0, True)
                        )  # @NOTE: is reward=0 correct? Probably the value doesn't matter.
                    elif letter in DIRS and a != DIRS[letter]:
                        li.append(
                            (self.wind_p, *self.__transition(row, col, DIRS[letter]))
                        )
                        li.append((1 - self.wind_p, *self.__transition(row, col, a)))
                    else:
                        if self.p_noise:
                            li.append(
                                (1 - self.p_noise, *self.__transition(row, col, a))
                            )
                            for b in (a_ for a_ in range(nA) if a_ != a):
                                li.append(
                                    (
                                        self.p_noise / (nA - 1),
                                        *self.__transition(row, col, b),
                                    )
                                )
                        else:
...
class SimpleGridEnvRLLib(SimpleGridEnv):
    ...
    def __init__(self, config):
        ...
        config["reward_map"] = {
            bytes(k, "utf-8"): v for k, v in config["reward_map"].items()
        }
...
def generate_random_map(size=8, p=0.8):
    ...
    DOES NOT CURRENTLY INCLUDE WIND
