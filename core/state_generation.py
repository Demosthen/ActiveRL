...
class BoundedUncertaintyMaximization(cooper.ConstrainedMinimizationProblem):
    ...
    def __init__(
        ...
        lower_bounded_idxs,
        upper_bounded_idxs,
        agent: UncertainPPOTorchPolicy,
        planning_model: LitPlanningModel = None,
        reward_model: RewardPredictor = None,
    ...
    def closure(self, resettable):
        ...
        if self.planning_model is None:
            ...
            planning_uncertainty, next_obs = (
                self.planning_model.compute_reward_uncertainty(
                    obs, action, return_avg_state=True
                )
            )
            ...
            loss = (
                -agent_uncertainty
                + self.planning_uncertainty_weight * planning_uncertainty
            ) / denom

        # Entries of p >= 0 (equiv. -p <= 0)
        loss += self.reg_coeff * torch.norm(
            resettable - self.original_resettable.detach()
        )
        ineq_defect = torch.cat(
            [
                resettable[self.lower_bounded_idxs] - self.lower_bounds,
                self.upper_bounds - resettable[self.upper_bounded_idxs],
            ]
        )
...
def get_space_bounds(obs_space: Space):
    ...
    if isinstance(obs_space, Box):
        ...
        return np.atleast_1d(obs_space.start), np.atleast_1d(
            obs_space.start + obs_space.n
        )
    else:
...
def sample_obs(
    ...
    resettable_part = torch.nn.Parameter(
        resettable_part.detach(), requires_grad=True
    )  # Make a leaf tensor that is an optimizable Parameter
...
def generate_states(
    ...
    if use_plr and len(env_buffer) > 0:
        ...
        return (
            plr_sample_state(env_buffer, plr_beta, curr_iter, rho=plr_rho),
            [0],
            "PLR",
        )

    obs, resettable = sample_obs(
        env,
        batch_size,
        agent.device,
        random=uniform_reset,
        naive_grounding=naive_grounding,
    )
