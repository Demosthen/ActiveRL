    ...
    for i, name in enumerate(col_names):
        ...
        if len(parts) > 1 and parts[0] == TAG:
            ...
            if parts[1] == "MAX":
                ...
                group_mins[grp] = group_mins[grp][group_mins[grp].iloc[:, 0] < cutoff]
                group_mins[grp].iloc[:, 1] = (
                    groups[orig_grp].iloc[:, 1] - group_mins[grp].iloc[:, 1]
                )

                group_maxs[grp] = df.iloc[:, [0, i]]
                group_maxs[grp] = group_maxs[grp]  # .fillna(method="ffill")
                group_maxs[grp] = group_maxs[grp][group_maxs[grp].iloc[:, 0] < cutoff]
                group_maxs[grp].iloc[:, 1] = (
                    groups[orig_grp].iloc[:, 1] + group_maxs[grp].iloc[:, 1]
                )
                print(
                    groups[orig_grp].shape, group_maxs[grp].shape, group_mins[grp].shape
                )
...
def draw(
    ...
    for col_name, df in combined:
        ...
        max_idx = (
            invalid_idxs.min() if len(invalid_idxs) > 0 else len(df)
        )  # get first index that becomes nan
    ...
    if plot_legend:
        ...
        figlegend.legend(
            curr_ax.get_legend_handles_labels()[0],
            curr_ax.get_legend_handles_labels()[1],
        )
        legend_fig_name = os.path.join(
            figs_dir, "{}_legend.{}".format(fig_name, format)
        )
        plt.tight_layout()
