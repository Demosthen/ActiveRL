...
def extract_rew_stats(run_dfs, all_bad_idxs, group=True):
    ...
    run_df_stes = (
        grouped_runs.std() / np.sqrt(cnt)
        if np.all(grouped_runs.count() > 1)
        else pd.DataFrame(
            np.zeros_like(run_df_means),
            index=run_df_means.index,
            columns=run_df_means.columns,
        )
    )
...
def plot_bars(start, rews, bad_idxs, graph_name, only_avg=False, relative=False):
    ...
    for i, tag in enumerate(valid_rew_keys):
        ...
        all_run_df_means, all_run_df_stes = extract_rew_stats(
            run_dfs, all_bad_idxs, group=False
        )
        ...
        if relative:
            ...
            xs = np.concatenate(
                [run_df_means.index, np.array([len(run_df_means.index)])]
            )
        ...
        if relative:
            rew_means = (
                100 * np.array(avg_rew_mean - BASELINES[tag]) / np.abs(BASELINES[tag])
            )
        ...
        rects = plt.bar(
            xs - total_width * num_tags / 2 + total_width * i,
            rew_means,
            yerr=rew_stes,
            width=width,
            align="center",
    ...
    extreme_env_labels = (
        ["Base", "Dry+Hot", "Wet+Windy", "Wet+Hot", "Dry+Cold", "Erratic", "Average"]
        if not only_avg
        else ["Base", "Average"]
    )
    ...
    if relative:
        ...
        plt.xticks(
            ...
            labels=extreme_env_labels[:num_xticks],
            fontsize=fontsize,
        )
