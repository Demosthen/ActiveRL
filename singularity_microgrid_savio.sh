...
singularity run --nv --workdir ./tmp --bind $(pwd):$HOME --bind "$LDIR:$HOME/.local" --bind "$LDIR:$HOME/.local" --bind "$PYTHON_DIR:/.env" austin_docker.sif sh -c "./singularity_preamble_new.sh && wandb agent social-game-rl/active-rl/$1 --count 1"
